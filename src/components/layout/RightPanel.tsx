...
export function RightPanel() {
    ...
    return (
        <div
            className={cn(
                "fixed inset-y-0 right-0 z-20 w-80 bg-background/80 backdrop-blur-xl border-l transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 flex flex-col",
                ...
                {selectedNodeId ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8 text-center space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground/50">
                            Select a service node on the canvas to view its configuration and metrics.
                        </p>
                    </div>
                )}
            </div>
